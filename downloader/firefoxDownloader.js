...
// "C:/Users/marti/Downloads" "H:/Light Novels"
const downloadPath = process.argv[2] || "C:/Users/marti/Downloads";
...
process.on('unhandledRejection', (err) => {
    if (err.message.includes('Requesting main frame too early')) {
      console.warn('Ignored fast popup error.');
      return;
    }
...
(async () => {
    ...
    const browser = await browserTypeWithExtension.launch({
        executablePath: "C:/Users/marti/AppData/Local/ms-playwright/firefox-1350/firefox/firefox.exe",
        headless: false,
    });
    let context = null;
    ...
    const navigateToDownloadPage = async ({title, href}) => {
        try {
            ...
            // const isTrue = await AreVolumesDownloaded(series.downloadedVolumes, series.availableVolumes);
            ...
            async function openLinks(links, watcher) {
                for (let i = 1; i < links.length + 1; i++) {
                    ...
                    if (recaptchaCheckbox) {
                        ...
                        for (const button of buttons) {
                            ...
                            if (!className.includes('disabled')) {
                                ...
                                if (isVisible) {
                                    ...
                                    console.log('Button is not visible on screen.');
                                }
                            } else {
                        ...
                        console.log(`Final button class: ${button_className}`);
                        ...
                        if (finalDownloadButton) {
                            ...
                            if (currentUrl.includes('drive.usercontent.google.com/download')) {
                                ...
                                try {
                                    ...
                                    const driveDownloadButton = await page.waitForSelector('.jfk-button-action', { visible: true, timeout: 3000 });
